...
tests, only compile/link checks and a shallow CI launch-and-screenshot smoke test, plus one narrow exception:
opener/popup behavior (`web-standards-tests/`) runs for real on both real-hardware CI runners and, separately,
a real 2015 Intel Mac over SSH (`scripts/macos-mac/`). `browser-windows-reactor`
